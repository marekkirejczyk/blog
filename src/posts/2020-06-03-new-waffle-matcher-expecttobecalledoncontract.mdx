...
}</pre>Now, with the new matcher, we can test it in the following ways. Firstly, examine if a specific function on the contract was called:

<pre>it('...', async () => {
 ...
 await contract.check();
 expect('balanceOf').to.be.calledOnContract(ERC20);
});</pre>Secondly, specify exact parameters to be used in the call:
...
});</pre>Full example [available here](https://github.com/EthWorks/Waffle/tree/master/examples/called-on-contract) .
